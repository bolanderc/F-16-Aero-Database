...
class Trim:
    ...
    def import_data(self, fn):
        ...
        if fn[-4:] == '.csv':
            ...
            self.data.to_csv("./TODatabase/TODatabase_linear_sorted.csv")
                                ...
                                for pp in range(num_pts):
                                    for qq in range(num_pts):
                                        for rr in range(num_pts):
                                            ...
                                            it += 1
            np.save("./TODatabase/TODatabase_linear.npy", self.data_array)
        elif fn[-4:] == '.npy':
...
#                params[i + 1] = np.sign(params[i + 1])*self.param_lims[i]
    ...
    def cpa_point_generator(self, input_fn):
        pnts = 10
        de_rng = np.linspace(-self.param_lims[2], self.param_lims[2], pnts)
        da_rng = np.linspace(-self.param_lims[3], self.param_lims[3], pnts)
        dr_rng = np.linspace(-self.param_lims[4], self.param_lims[4], pnts)
        Cl_cpa = np.zeros((pnts, pnts, pnts))
        Cm_cpa = np.zeros((pnts, pnts, pnts))
        Cn_cpa = np.zeros((pnts, pnts, pnts))
        alpha_t = self.trim_solution[1]
        beta_t = self.trim_solution[2]
        rates = [self.p_trim, self.q_trim, self.r_trim]
        mx_scene = mx.Scene(input_fn)
        ...
        for i in range(pnts):
            for j in range(pnts):
                for k in range(pnts):
                    mx_scene.set_aircraft_state(state={"alpha": alpha_t,
                                                       "beta": beta_t,
                                                       "angular_rates": rates,
                                                       "velocity": 222.5211})
                    mx_scene.set_aircraft_control_state(control_state={"elevator": de_rng[i],
                                                                       "aileron": da_rng[j],
                                                                       "rudder": dr_rng[k]})
                    x = mx_scene.solve_forces(**forces_options)["F16"]["total"]
                    Cl_cpa[i, j, k] = x["Cl"]
                    Cm_cpa[i, j, k] = x["Cm"]
                    Cn_cpa[i, j, k] = x["Cn"]
...
V = 222.5211
case = Trim(V, rho)
case.import_data("./TODatabase/TODatabase_linear.npy")
case.general_trimmer(theta=theta, phi=phi)
case.cpa_point_generator("./TODatabase/F16_input.json")
